...
public class Crypt {
	 ...
	 */
	public Crypt() {
		this.defaultIV = new IvParameterSpec(Crypt.defaultIVBytes);
	}
	 ...
	 */
	public Crypt(byte[] initializationVector) {
	...
	public byte[] encrypt(byte[] data) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException {
		...
		if (data.length < this.keySize - 11) {
			...
			byte keyLength = (byte) (encryptedKey.length >> Crypt.keyLengthMultiplier);
		
			encryptedData = new byte[Crypt.keyLengthFieldSize + encryptedKey.length + aesCrypted.length];
			
			encryptedData[0] = keyLength;
			System.arraycopy(encryptedKey, 0, encryptedData, Crypt.keyLengthFieldSize, encryptedKey.length);
			System.arraycopy(aesCrypted, 0, encryptedData, (Crypt.keyLengthFieldSize + encryptedKey.length), aesCrypted.length);
		}
	...
	public byte[] decrypt(byte[] data) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException {
		...
		if ((prefix & Crypt.methodMask) == Crypt.methodMask) {
			// hybrid mode was used for encryption
			// get AES key length
			int keyLengthInBytes = cryptedData[0] << Crypt.keyLengthMultiplier;
			
			// get AES key
			byte[] encryptedKey = new byte[keyLengthInBytes];
			System.arraycopy(cryptedData, Crypt.keyLengthFieldSize, encryptedKey, 0, keyLengthInBytes);
			byte[] key = this.doRSADecryption(encryptedKey);
			
			// decrypt data
			byte[] aesData = new byte[cryptedData.length - keyLengthInBytes - Crypt.keyLengthFieldSize];
			System.arraycopy(cryptedData, (Crypt.keyLengthFieldSize + keyLengthInBytes), aesData, 0, aesData.length);

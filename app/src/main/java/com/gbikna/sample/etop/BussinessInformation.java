...
public class BussinessInformation extends AppCompatActivity {
    ...
    String[] processStates()
    {
        ...
        try {
            ...
            for(i = 0; i < arr.length(); i++)
            {
                ...
                JSONObject obj2 = obj.getJSONObject("state");
                option[i] = obj2.getString("name");
            }
    ...
    String[] lgas = new String[50];
    String[] lgaids = new String[50];
    void processLgaCity(String state)
    {
        int i = 0, j = 0;
        if(state.isEmpty())
        {
        ...
        try {
            ...
            for(i = 0; i < arr.length(); i++)
            {
                JSONObject obj = arr.getJSONObject(i);
                JSONObject obj2 = obj.getJSONObject("state");
                if(obj2.getString("name").equals(state))
                {
                    JSONArray lc = obj2.getJSONArray("locals");
                    int k = 0;
                    for(k = 0; k < lc.length(); k++)
                    {
                        JSONObject bv = lc.getJSONObject(k);
                        cities[j] = bv.getString("name");
                        lgas[j] = bv.getString("name");
                        lgaids[j] = String.valueOf(bv.getInt("id"));
                        j++;
                    }
                    return;
                }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.setClass(BussinessInformation.this, Login.class);
        ...
        businessaddress = (TextInputLayout) findViewById(R.id.businessaddress);
        businessname = (TextInputLayout) findViewById(R.id.businessname);
        businessphone = (TextInputLayout) findViewById(R.id.businessphone);
        businesstype = (TextInputLayout) findViewById(R.id.businesstype);
        cac = (TextInputLayout) findViewById(R.id.cac);
        landmark = (TextInputLayout) findViewById(R.id.landmark);
        ...
        lga = (AutoCompleteTextView) findViewById(R.id.lga);
        ...
        if(BussinessInformation.busControl == 1)
        {
            businessaddress.getEditText().setText(SignupVr.businessaddress);
            businessname.getEditText().setText(SignupVr.businessname);
            businessphone.getEditText().setText(SignupVr.businessphone);
            businesstype.getEditText().setText(SignupVr.businesstype);
            cac.getEditText().setText(SignupVr.cacnumber);
            state.setText(SignupVr.state);
            lga.setText(SignupVr.lga);
            city.setText(SignupVr.city);
            landmark.getEditText().setText(SignupVr.landmark);
            BussinessInformation.busControl = 0;
        }

        state.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> p, View v, int pos, long id) {
                Log.i("DATA", "Selected State: " + state.getText().toString());
                Log.i("DATA", "Selected Cities: " + cities.toString());
                if(!state.getText().toString().isEmpty())
                {
                    processLgaCity(state.getText().toString());
    ...
    public void Proceed() {
        SignupVr.businessaddress = businessaddress.getEditText().getText().toString().trim();
        SignupVr.businessname = businessname.getEditText().getText().toString().trim();
        SignupVr.businessphone = businessphone.getEditText().getText().toString().trim();
        SignupVr.businesstype = businesstype.getEditText().getText().toString().trim();
        SignupVr.cacnumber = cac.getEditText().getText().toString().trim();
        SignupVr.landmark = landmark.getEditText().getText().toString().trim();

        SignupVr.city = city.getText().toString().trim();
        SignupVr.lga = lga.getText().toString().trim();
        SignupVr.state = state.getText().toString();
        SignupVr.lgacode = "";


        if (SignupVr.businessaddress.isEmpty()) {
            businessaddress.setFocusable(true);
            Toast.makeText(BussinessInformation.this, "Provide Business Address", Toast.LENGTH_LONG).show();
        } else if (SignupVr.businessname.isEmpty()) {
            businessname.setFocusable(true);
            Toast.makeText(BussinessInformation.this, "Provide Business Name", Toast.LENGTH_LONG).show();
        } else if (SignupVr.businessphone.isEmpty()) {
            businessphone.setFocusable(true);
            Toast.makeText(BussinessInformation.this, "Provide Business Phone", Toast.LENGTH_LONG).show();
        } else if (SignupVr.businesstype.isEmpty()) {
            businesstype.setFocusable(true);
            Toast.makeText(BussinessInformation.this, "Provide Business Type", Toast.LENGTH_LONG).show();
        } else if(SignupVr.state.isEmpty())
        {
            state.setFocusable(true);
            Toast.makeText(BussinessInformation.this, "Select States", Toast.LENGTH_LONG).show();
        } else if(SignupVr.lga.isEmpty())
        {
            lga.setFocusable(true);
            Toast.makeText(BussinessInformation.this, "Select Lga", Toast.LENGTH_LONG).show();
        } else if(SignupVr.city.isEmpty())
        {
            city.setFocusable(true);
            Toast.makeText(BussinessInformation.this, "Select City", Toast.LENGTH_LONG).show();
        } else if(SignupVr.landmark.isEmpty())
        {
            landmark.setFocusable(true);
            Toast.makeText(BussinessInformation.this, "Provide Closest Landmark", Toast.LENGTH_LONG).show();
        } else {
            int lv = Arrays.asList(cities).indexOf(SignupVr.lga);
            if(Profile.controller != 1)
                SignupVr.lgacode = lgaids[lv];
            Intent inte = new Intent();
            inte.setClass(BussinessInformation.this, PersonalInformation.class);
            startActivity(inte);
            finish();
        }
    }

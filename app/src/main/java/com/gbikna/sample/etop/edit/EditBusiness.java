...
public class EditBusiness extends AppCompatActivity {
    ...
    private static final String TAG = EditBusiness.class.getSimpleName();
    Button button;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.setClass(EditBusiness.this, Dashboard.class);
        ...
        }else
        {
            ...
            businessphone.setEnabled(false);
    ...
    public void Proceed() {
        String baddr = businessaddress.getEditText().getText().toString().trim();
        String bname = businessname.getEditText().getText().toString().trim();
        String bphone = businessphone.getEditText().getText().toString().trim();
        String btype = businesstype.getEditText().getText().toString().trim();
        String cacc = cac.getEditText().getText().toString().trim();
        String lmark = landmark.getEditText().getText().toString().trim();
        String ccity = city.getText().toString().trim();
        String llga = lga.getText().toString().trim();
        String sstate = state.getText().toString();
        String slga = "NA";

        if(baddr.isEmpty() || bname.isEmpty() || bphone.isEmpty() || btype.isEmpty()
        || cacc.isEmpty() || lmark.isEmpty() || ccity.isEmpty() || llga.isEmpty()
        || sstate.isEmpty() || slga.isEmpty())
        {
            Toast.makeText(EditBusiness.this, "PROVIDE MISSING INFORMATION", Toast.LENGTH_LONG).show();
        } else {
            int lv = Arrays.asList(cities).indexOf(SignupVr.lga);
            String lgid = lgaids[lv];
            ...
            try {
                send.put("city", ccity);
                send.put("lga", llga);
                send.put("lgacode", lgid);
                send.put("state", sstate);
                send.put("businessaddress", baddr);
                send.put("businessname", bname);
                send.put("businessphone", bphone);
                send.put("businesstype", btype);
                send.put("cacnumber", cacc);
                send.put("latitude", "");
                send.put("longitude", "");
                send.put("serialnumber", Utilities.getSerialNumber());
                send.put("landmark", lmark);
                Log.i(TAG, "REQUEST: " + send.toString());
                new updateBusinessInfo().execute(BASEURL + "/apis/etop/signup/business", send.toString());
            } catch (JSONException e) {
                e.printStackTrace();
    ...
    public class updateBusinessInfo extends AsyncTask<String, Void, String> {
                        ...
                        public void run() {
                            ...
                            try {
                                ...
                                if(obj.has("status") && obj.getInt("status") == 200)
                                {
                                    ...
                                    Intent intent = new Intent();
                                    intent.setClass(EditBusiness.this, MainActivity.class);
                                    startActivity(intent);
                                    finish();
                                }else
                                {
                                    ...
                                    Toast.makeText(EditBusiness.this, "BUSINESS UPDATE FAILED", Toast.LENGTH_LONG).show();
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                                Toast.makeText(EditBusiness.this, "BUSINESS UPDATE FAILED", Toast.LENGTH_LONG).show();
